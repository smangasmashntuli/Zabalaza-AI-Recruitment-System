...
def add_missing_columns():
    ...
    # Handle saved_jobs constraint removal
    print("\nChecking saved_jobs table constraints...\n")
    with engine.begin() as connection:
        try:
            # Check if the unique constraint exists
            inspector = inspect(engine)
            if inspector.has_table('saved_jobs'):
                constraints = inspector.get_unique_constraints('saved_jobs')
                constraint_names = [c['name'] for c in constraints]
                
                if 'uix_candidate_saved_job' in constraint_names:
                    print("Removing old unique constraint 'uix_candidate_saved_job'...")
                    connection.execute(text("ALTER TABLE `saved_jobs` DROP CONSTRAINT `uix_candidate_saved_job`;"))
