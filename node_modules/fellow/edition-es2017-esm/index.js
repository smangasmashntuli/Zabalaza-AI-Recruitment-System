...
/** GitHub Gist URL */
function getUsernameFromGitHubGistUrl(url) {
...
/** GitHub Website URL */
function getUsernameFromGitHubWebsiteUrl(url) {
...
class Fellow {
    ...
    set githubUrl(input) {
        const username = getUsernameFromGitHubUrl(input) ||
        ...
        if (username) {
            if (githubInvalidUsernames.includes(username.toLowerCase())) {
                throw new Error(`Invalid GitHub Profile URL containing reserved namespace: ${input}`);
            }
            else {
    ...
    set(fellow) {
        ...
        if (typeof fellow === 'string') {
            const match = /^(?<yearsAndName>[^:<(]+)\s*(?:<(?<email>.*?)>)?\s*(?<urls>(?:\(.*?\)\s*)*)\s*(?::\s*(?<description>.*))?$/.exec(trim(fellow));
            if (!match) {
                throw new Error('Invalid fellow string');
            }
            const yearsAndName = trim((_a = match.groups) === null || _a === void 0 ? void 0 : _a.yearsAndName);
            if (yearsAndName)
                this.name = yearsAndName;
            const email = trim((_b = match.groups) === null || _b === void 0 ? void 0 : _b.email);
            if (email)
                this.email = email;
            const urls = (((_d = (_c = match.groups) === null || _c === void 0 ? void 0 : _c.urls) === null || _d === void 0 ? void 0 : _d.split(/\s+/)) || [])
                .map((url) => trim(url || '').replace(/^\(|\)$/g, ''))
                .filter(Boolean);
            for (const url of urls) {
                this.url = url;
            }
            const description = trim((_e = match.groups) === null || _e === void 0 ? void 0 : _e.description);
            if (description)
